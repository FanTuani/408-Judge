...
成功评审后，扩展会在被评审 `.c` 或 `.cpp` 文件所在目录旁的 `.408judge/` 文件夹中保存本次源码快照、评审结论和时间；每个源码文件使用一个独立的 JSON 历史文件。该文件夹不会由扩展自动上传，但可能被用户自己的版本控制、云盘或同步工具处理；不希望同步时，应将 `.408judge/` 加入项目的 `.gitignore` 或相应排除规则。
