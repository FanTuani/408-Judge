...
function thinkingStatusContent(status: ThinkingStatus): string {
  const text = status.complete ? `${status.label} · ${(status.elapsedMs / 1000).toFixed(1)} 秒` : status.label;
  return `<div class="stream-status${status.complete ? ' thinking-complete' : ''}"><span id="thinking-spinner" class="spinner"${status.complete ? ' hidden' : ''}></span><span id="thinking-check" class="thinking-check"${status.complete ? '' : ' hidden'}>✓</span><strong id="thinking-label">${escapeHtml(text)}</strong><span id="attempt">${status.attempt > 1 ? `重试 ${status.attempt}/2` : ''}</span></div>`;
}

export function renderWebview(state: ViewState, nonce: string, celebrateCorrect = false): string {
  let body: string;
  if (state.kind === 'loading') {
    ...
    body = `<header><span class="eyebrow">${escapeHtml(state.fileName)}</span><button id="review" class="secondary">重新评审</button></header>${state.thinkingStatus ? thinkingStatusContent(state.thinkingStatus) : ''}${resultContent(state.source, state.result)}`;
  } else {
  ...
  <style>
    ...
    function launchFireworks(){
      ...
      if(!badge)return;
      const layer=document.createElement('div');layer.className='fireworks-layer';layer.setAttribute('aria-hidden','true');document.body.appendChild(layer);
      const rect=badge.getBoundingClientRect();const width=window.innerWidth;const baseY=Math.max(58,rect.top-18);
      const bursts=[{x:rect.left+rect.width/2,y:baseY},{x:width*.28,y:Math.max(64,baseY-38)},{x:width*.72,y:Math.max(72,baseY-16)}];
      const colors=['#ffd166','#06d6a0','#4cc9f0','#f72585','#ff9f1c','#b8f2e6'];
      bursts.forEach((burst,burstIndex)=>{
        const delay=burstIndex*140;const ring=document.createElement('i');ring.className='firework-ring';ring.style.setProperty('--x',burst.x+'px');ring.style.setProperty('--y',burst.y+'px');ring.style.setProperty('--color',colors[burstIndex*2]);ring.style.setProperty('--delay',delay+'ms');layer.appendChild(ring);
        for(let index=0;index<18;index+=1){const angle=(Math.PI*2*index/18)+(burstIndex*.17);const distance=28+Math.random()*34;const particle=document.createElement('i');particle.className='firework-particle';particle.style.setProperty('--x',burst.x+'px');particle.style.setProperty('--y',burst.y+'px');particle.style.setProperty('--tx',(Math.cos(angle)*distance)+'px');particle.style.setProperty('--ty',(Math.sin(angle)*distance+18)+'px');particle.style.setProperty('--color',colors[(index+burstIndex*2)%colors.length]);particle.style.setProperty('--delay',(delay+Math.random()*55)+'ms');layer.appendChild(particle);}
      });
      setTimeout(()=>layer.remove(),1700);
    }
    window.addEventListener('message',event=>{if(event.data?.type!=='stream')return;const label=document.getElementById('thinking-label');const spinner=document.getElementById('thinking-spinner');const check=document.getElementById('thinking-check');const preview=document.getElementById('structured-preview');const attempt=document.getElementById('attempt');if(label&&event.data.thinkingText)label.textContent=event.data.thinkingText;if(spinner)spinner.hidden=Boolean(event.data.thinkingComplete);if(check)check.hidden=!event.data.thinkingComplete;if(preview)preview.innerHTML=event.data.previewHtml;if(attempt)attempt.textContent=event.data.attempt>1?('重试 '+event.data.attempt+'/2'):'';if(event.data.celebrateCorrect)launchFireworks();});
    if(${celebrateCorrect ? 'true' : 'false'})queueMicrotask(launchFireworks);
  </script></body></html>`;
}
...
export class JudgeViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
    ...
    if (this.view && previous.kind === 'loading' && state.kind === 'loading') {
      const thinkingText = state.thinkingStatus
        ? state.thinkingStatus.complete
          ? `${state.thinkingStatus.label} · ${(state.thinkingStatus.elapsedMs / 1000).toFixed(1)} 秒`
          : state.thinkingStatus.label
        : undefined;
      const celebrateCorrect = this.consumeCorrectCelebration();
      void this.view.webview.postMessage({
        type: 'stream', thinkingText, thinkingComplete: state.thinkingStatus?.complete ?? false,
  ...
  private render(): void {
    ...
    this.view.webview.html = renderWebview(this.state, nonce, this.consumeCorrectCelebration());
  }
