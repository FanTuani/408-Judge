...
export function renderWebview(state: ViewState, nonce: string, celebrateCorrect = false, confettiScriptUri = '', cspSource = ''): string {
  let body: string;
  if (state.kind === 'loading') {
    ...
    body = `<header><span class="eyebrow">${escapeHtml(state.fileName)}</span><button id="review" class="secondary">重新评审</button></header>${state.thinkingStatus ? thinkingStatusContent(state.thinkingStatus) : ''}<section class="live-preview final-result">${resultContent(state.source, state.result)}</section>`;
  } else {
...
export class JudgeViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  private render(): void {
    ...
    this.view.webview.html = renderWebview(this.state, nonce, this.consumeCorrectCelebration(), confettiScriptUri, this.view.webview.cspSource);
  }
