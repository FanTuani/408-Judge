...
function reviewControls(label: string, thinkingLevel: ThinkingLevel, secondary = false): string {
  return `<div class="review-actions"><button id="review"${secondary ? ' class="secondary"' : ''}>${label}</button><select id="thinking-level" aria-label="思考强度" title="选择下一次评审的思考强度"><option value="disabled"${thinkingLevel === 'disabled' ? ' selected' : ''}>思考：关闭</option><option value="high"${thinkingLevel === 'high' ? ' selected' : ''}>思考：高强度</option><option value="max"${thinkingLevel === 'max' ? ' selected' : ''}>思考：最大强度</option></select></div>`;
}
...
export function renderWebview(state: ViewState, nonce: string, celebrateCorrect = false, confettiScriptUri = '', cspSource = '', thinkingLevel: ThinkingLevel = 'high'): string {
  ...
  if (state.kind === 'loading') {
    body = `<header><span class="eyebrow">${escapeHtml(state.fileName)}</span><button id="cancel" class="secondary">取消</button></header>
      ...
      <section class="live-preview" aria-live="polite"><div id="structured-preview">${previewContent(state.source, state.preview)}</div></section>`;
    ...
    body = `<header><span class="eyebrow">${escapeHtml(state.fileName)}</span>${reviewControls('重新评审', thinkingLevel, true)}</header>${state.thinkingStatus ? thinkingStatusContent(state.thinkingStatus) : ''}<section class="live-preview final-result">${resultContent(state.source, state.result)}</section>`;
  } else {
