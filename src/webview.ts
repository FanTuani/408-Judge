...
export function renderWebview(state: ViewState, nonce: string, celebrateCorrect = false, confettiScriptUri = ''): string {
  let body: string;
  ...
  <style>
    ...
    function launchConfetti(){
      ...
      const rect=badge.getBoundingClientRect();const width=window.innerWidth;const height=window.innerHeight;
      window.launchJudgeConfetti({x:Math.max(.08,Math.min(.92,(rect.left+rect.width/2)/width)),y:Math.max(.58,Math.min(.72,(rect.bottom+height*.22)/height))});
    }
...
export class JudgeViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  private render(): void {
    ...
    this.view.webview.html = renderWebview(this.state, nonce, this.consumeCorrectCelebration(), confettiScriptUri);
  }
