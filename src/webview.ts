...
function reviewControls(label: string, thinkingLevel: ThinkingLevel, secondary = false, showThinkingLevel = true, shortcut?: string): string {
  const classes = `review-button${secondary ? ' secondary' : ''}`;
  return `<div class="review-actions"><button id="review" class="${classes}"><span>${escapeHtml(label)}</span>${shortcutHint(shortcut)}</button><button id="history" class="secondary history-button">历史记录</button>${showThinkingLevel ? thinkingLevelSelect(thinkingLevel) : ''}</div>`;
}
...
export function renderWebview(state: ViewState, nonce: string, celebrateCorrect = false, confettiScriptUri = '', cspSource = '', thinkingLevel: ThinkingLevel = 'high', reviewShortcut?: string): string {
  let body: string;
  if (state.kind === 'loading') {
    ...
    body = `<div class="center"><div class="state-icon">!</div><h1>无法完成评审</h1><p>${escapeHtml(state.message)}</p>${reviewControls('重新评审', thinkingLevel, false, true, reviewShortcut)}</div>`;
  } else if (state.kind === 'result') {
    body = `<header><span class="eyebrow">${escapeHtml(state.fileName)}</span><div id="header-actions">${reviewControls('重新评审', thinkingLevel, true, !state.thinkingStatus, reviewShortcut)}</div></header>${state.thinkingStatus ? thinkingStatusContent(state.thinkingStatus, thinkingLevel) : ''}<section class="live-preview final-result result-stack">${resultContent(state.source, state.result)}</section>`;
    ...
    body = `<div class="center"><div class="state-icon">✓</div><h1>408 Judge</h1><p>${escapeHtml(state.message ?? '打开一道 .cpp 作答，然后从编辑器右键菜单开始评审。')}</p>${reviewControls('评审当前 C++ 作答', thinkingLevel, false, true, reviewShortcut)}</div>`;
  }
...
export class JudgeViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    this.disposables.push(view.webview.onDidReceiveMessage((message: unknown) => {
      ...
      const data = message as { type?: string; line?: number; level?: string; id?: string; fileUri?: string };
  ...
  setState(state: ViewState): void {
    ...
    if (this.view && previous.kind === 'loading' && state.kind === 'result') {
      ...
      void this.view.webview.postMessage({
        ...
        headerActionsHtml: reviewControls('重新评审', this.thinkingLevel, true, !state.thinkingStatus, this.reviewShortcut),
  ...
  private render(): void {
    ...
    this.view.webview.html = renderWebview(this.state, nonce, this.consumeCorrectCelebration(), confettiScriptUri, this.view.webview.cspSource, this.thinkingLevel, this.reviewShortcut);
  }
