...
function thinkingStatusContent(status: ThinkingStatus, thinkingLevel?: ThinkingLevel): string {
  ...
  const stages = status.stages.map(stage => `<li>${escapeHtml(stage)}</li>`).join('');
...
export function renderWebview(state: ViewState, nonce: string, celebrateCorrect = false, confettiScriptUri = '', cspSource = '', thinkingLevel: ThinkingLevel = 'high'): string {
  ...
  <style>
    ...
    window.addEventListener('message',event=>{if(event.data?.type!=='stream')return;const spinner=document.getElementById('thinking-spinner');const check=document.getElementById('thinking-check');const preview=document.getElementById('structured-preview');const attempt=document.getElementById('attempt');const stages=document.getElementById('thinking-stages');if(thinkingLabel&&typeof event.data.thinkingElapsedMs==='number'){thinkingElapsed=event.data.thinkingElapsedMs;thinkingAnchor=Date.now();thinkingDone=Boolean(event.data.thinkingComplete);thinkingLabel.textContent=thinkingDone?event.data.thinkingText:('Thinking · '+Math.floor(thinkingElapsed/1000)+' 秒')}if(spinner)spinner.hidden=Boolean(event.data.thinkingComplete);if(check)check.hidden=!event.data.thinkingComplete;if(event.data.thinkingComplete)stages?.remove();else if(stages&&Array.isArray(event.data.thinkingStages)){const incoming=event.data.thinkingStages;let common=0;while(common<stages.children.length&&common<incoming.length&&stages.children[common].textContent===incoming[common])common++;while(stages.children.length>common)stages.lastElementChild?.remove();for(let i=common;i<incoming.length;i++){const item=document.createElement('li');item.textContent=incoming[i];stages.appendChild(item)}if(incoming.length>common)stages.scrollTop=stages.scrollHeight}if(preview)preview.innerHTML=event.data.previewHtml;if(attempt)attempt.textContent=event.data.attempt>1?('重试 '+event.data.attempt+'/2'):'';if(event.data.celebrateCorrect)launchConfetti();});
