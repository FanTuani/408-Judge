...
每次成功评审后，插件都会把当时的源码快照和完整结论保存到工作区根目录的 `.408judge` 文件夹。文件夹内部按源码的相对路径组织，每个 `.cpp` 对应一个独立的 JSON 历史文件，例如 `src/tree.cpp` 会保存到 `.408judge/src/tree.cpp.json`。点击评审按钮下方的“历史记录”，可以统一按时间查看这些评测，并打开任意一条完整结果。每个 `.cpp` 最多保留最近 20 条；如果当前文件不属于任何 VS Code 工作区，则 `.408judge` 文件夹保存在该 `.cpp` 所在目录。
