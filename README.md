...
把题干和作答写进 `.c` 或 `.cpp`，右键就能开始评测。408 Judge 会结合题意检查算法逻辑、边界条件和复杂度，告诉你思路是否成立，以及哪里需要修改。
...
`.c` 或 `.cpp` 文件应当以一段题干注释开头，写清题目要求、输入输出和必要约束。注释下面紧接你的作答，例如：
...
1. 在编辑器中打开准备好的 `.c` 或 `.cpp` 文件。
2. 按 `Cmd+'`（macOS）或 `Ctrl+'`（Windows/Linux），也可以右键选择“408 Judge: 评审当前 C/C++ 作答”。
3. 在 Activity Bar 打开 “408 Judge” 查看进度和结果。

侧边栏的评审按钮会显示当前快捷键；在 VS Code“键盘快捷方式”中重新绑定或移除后，按钮提示会自动同步。再次评测会自动取消旧请求，侧边栏也可以随时取消当前评测。

底部工具栏提供主页、历史记录和设置入口，模型与思考强度选择器固定在右侧；其中“设置”会直接打开 408 Judge 的插件设置页面。

### 4. 查看历史记录

每次成功评审后，插件都会把当时的源码快照和完整结论保存到该 `.c` 或 `.cpp` 所在目录旁的 `.408judge` 文件夹。每个源码文件对应一个独立的 JSON 历史文件，文件名由源码文件名和完整路径哈希组成；例如 `src/tree.c` 的记录会保存在 `src/.408judge/.tree.c_<hash>.json`。这种布局与 CPH 类似，同一个工作区内可以存在多个 `.408judge` 文件夹。点击侧边栏底部工具栏的“历史记录”，插件会统一扫描并按时间展示这些评测，也能打开任意一条完整结果。每个源码文件最多保留最近 20 条。
...
发出请求后，侧边栏先用 `Pending · N 秒` 显示等待首段响应的时间；API 返回首段有效信息后再从 `Thinking · 0 秒` 开始计算思考时间，并且每 10 秒生成一段阶段概括。完整结果准备好后再展开，不会展示模型的原始思维链。
...
| 设置 | 默认值 | 用途 |
| --- | --- | --- |
| `deepseekJudge.model` | `deepseek-v4-pro` | 主评审模型；底栏内置 Flash 和 Pro 快速选择 |
...
右下角可以直接选择 Flash 或 Pro，并分别搭配关闭、高强度、最大强度，共六种组合。更高的思考强度通常需要更多时间，也可能产生更多 API 用量。设置从下一次评测开始生效。

## 数据与隐私

扩展只在你主动评测时发送当前 C/C++ 作答、可选的同名 Markdown 和附加要求。它不会上传工作区中的其他文件、修改源码或收集遥测。评测历史只写入本地 `.408judge/`；如果不希望将源码快照提交到版本控制，请把该文件夹加入项目的 `.gitignore`。数据处理方式以 DeepSeek 或你配置的 API 服务条款为准，详见 [隐私说明](PRIVACY.md)。
