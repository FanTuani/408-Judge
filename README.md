...
每次成功评审后，插件都会把当时的源码快照和完整结论保存到该 `.cpp` 所在目录旁的 `.408judge` 文件夹。每个 `.cpp` 对应一个独立的 JSON 历史文件，文件名由源码文件名和完整路径哈希组成；例如 `src/tree.cpp` 的记录会保存在 `src/.408judge/.tree.cpp_<hash>.json`。这种布局与 CPH 类似，同一个工作区内可以存在多个 `.408judge` 文件夹。点击评审按钮下方的“历史记录”，插件会统一扫描并按时间展示这些评测，也能打开任意一条完整结果。每个 `.cpp` 最多保留最近 20 条。
...
扩展只在你主动评测时发送当前 C++ 作答、可选的同名 Markdown 和附加要求。它不会上传工作区中的其他文件、修改源码或收集遥测。评测历史只写入本地 `.408judge/`；如果不希望将源码快照提交到版本控制，请把该文件夹加入项目的 `.gitignore`。数据处理方式以 DeepSeek 或你配置的 API 服务条款为准，详见 [隐私说明](PRIVACY.md)。
