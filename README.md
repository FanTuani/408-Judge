...
1. 打开作答 `.cpp`。同目录存在同名讲解时会自动参考，例如 `tree.cpp` / `tree.md` 或 `tree_exam_2024.cpp` / `tree_exam_2024.md`；没有 `.md` 也可继续评测。
...
4. 在 Activity Bar 的 “408 Judge” 中查看类似 ChatGPT 的思考阶段时间线：顶部显示 `Thinking` 和实时更新的用时，思考期间界面只展示阶段列表。每个阶段包含简短动宾标题和一至两行流式补全的说明；标题完整生成后才创建节点，相同或相近阶段只更新当前节点，任务焦点明显变化时才向下追加。列表随内容自然增长，不使用内部滚动条；停留在页面底部时自动跟随新内容，手动向上翻阅后暂停跟随。界面不展示原始思维链。首次收到结论时阶段列表自动收起，只保留并冻结 `思考完成 · 用时`。结论 JSON 到达时会被增量解析，直接更新 verdict、总体评价、正确之处、问题、复杂度和最小修复，无需等待整个响应结束。当流式 verdict 首次确定为 `correct` 时，会立即从画面中央向上播放一次增强版 `canvas-confetti` Basic Cannon；彩纸使用 Webview 内的主线程专用 Canvas 直接喷射、绽放并自然下落，不依赖 CSP 受限的 Worker。系统开启“减少动态效果”时自动禁用。带行号的问题可点击回到源码。

评测前或评测完成后，可在侧边栏直接切换“思考：关闭 / 高强度 / 最大强度”。评测完成后，下拉框位于“思考完成 · 用时”同一行右侧；尚无思考状态时则位于评审按钮下方。选择会立即保存到 VS Code 配置，并从下一次评测开始生效；正在进行的请求不会被中途改变。

必要的局部修正会要求模型同时返回原始代码块与最小连续行替换。扩展先在源码中精确定位原始块，再消除首尾未改变的行，以标准统一 diff 展示删除行、增加行和少量上下文；不会直接修改源码。

扩展不会写入 Problems、生成报告、修改源码或上传题库中的其他文件。每次只评审当前 `.cpp` 和可选的同名 `.md`。参考讲解作为评审依据之一，而不是唯一答案；没有讲解或题干注释时，模型会根据函数签名、参数、数据结构和代码行为做保守推断，并在总结中说明关键假设和不确定性，而不会仅因上下文缺失拒绝评测。结果不展示未经校准的模型自评置信度。
