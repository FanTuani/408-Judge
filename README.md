...
扩展不会写入 Problems、生成报告、修改源码或上传题库中的其他文件。每次只评审当前 `.cpp` 和可选的同名 `.md`。参考讲解作为评审依据之一，而不是唯一答案；没有讲解或题干注释时，模型会根据函数签名、参数、数据结构和代码行为做保守推断，在总结中说明关键假设并降低置信度，而不会仅因上下文缺失拒绝评测。
...
```sh
code --install-extension ./408-judge-0.11.7.vsix
```
